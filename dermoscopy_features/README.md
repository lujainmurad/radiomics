...
## Project Structure
    ...
    README.md                      This file
    data/masked_images/            Input: original x binary mask images
        train/                     2000 images
        val/                       150 images
        test/                      600 images
    data/predicted_masks_clean/    Input: binary segmentation masks
        train/, val/, test/
    outputs/
        features.csv               All 2750 images, 234 features
        features_train.csv         Train split (2000 rows)
        features_val.csv           Val split (150 rows)
        features_test.csv          Test split (600 rows)
...
If bashrc was set up, venv activates automatically and you will see:
    venv310 activated
...
Run these 4 steps before closing:
    ...
    # Step 4 - download CSVs to your computer
    Right-click in VS Code Explorer and Download:
...
## If Codespace Gets Deleted (30 plus days inactive)

Reinstall everything IN THIS EXACT ORDER:
...
- PyRadiomics requires Python 3.10 — does NOT work on Python 3.12 (Codespace default)
- numpy must be pinned to 1.26.4 — pyradiomics was compiled against numpy 1.x
- opencv must be headless version — no display needed in Codespaces
- Always use --workers 1 when debugging to see errors clearly
- data/ folder is NOT committed to git (too large) — only CSVs and scripts are committed
- Codespace persists for 30 days of inactivity before deletion
